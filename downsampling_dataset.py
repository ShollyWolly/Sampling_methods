...
stratified = stratified_sample(df=input_csv, strata=['Sex', 'LAND'], size=0.3)


input_csv['Stratify'] = input_csv['Sex'] + ", " + input_csv['LAND']
stratified['Stratify'] = stratified['Sex'] + ", " + stratified['LAND']

rel_input = (input_csv['Stratify'].value_counts() / len(input_csv)).sort_values(ascending=False)
new_propo = (stratified['Stratify'].value_counts() / len(stratified)).sort_values(ascending=False)
